...
class Pedestrian (Supervisor):
    ...
    def run(self):
        ...
        # Wenn Webots keine Route übergibt, nutzen wir einfach unsere eigene fest einprogrammierte Route:
        if not options.trajectory or len(options.trajectory.split(',')) < 2:
            print("Verwende fest einprogrammierte Standard-Route!")
            # Du kannst diese Zahlenpaare (X Y) beliebig anpassen.
            # Zwei Punkte ergeben eine Hin- und Her-Bewegung (dynamisches Hindernis):
            options.trajectory = "0 -5.5, 0 -1"
        # -----------------------------
        ...
        while not self.step(self.time_step) == -1:
            ...
            # / 'distance between way-point(X) and way-point(X+1)'
